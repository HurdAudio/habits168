...
router.get('/assembled/:user_uuid', (req, res, next) => {
    ...
    knex('thursday_subscriptions')
        ...
        .first()
        .then(userThursdayFeeds => {
            if (!userThursdayFeeds) {
                return next();
            }
            lastAccessed = new Date(userThursdayFeeds.updated_at);
                            ...
                            .then(blogFeeds => {
                                knex('podcast_feeds')
                                    ...
                                    .then(podcastFeeds => {
                                        ...
                                        for (let i = 0; i < userThursdayFeeds.tabs.tabs.length; i++) {
                                            result[i + 1] = {
                                                active: userThursdayFeeds.tabs.tabs[i].active,
                                                allRead: userThursdayFeeds.tabs.tabs[i].allRead,
                                                podcast: userThursdayFeeds.tabs.tabs[i].podcast,
                                                sort: userThursdayFeeds.tabs.tabs[i].sort,
                                                subscriptions: [],
                                                title: userThursdayFeeds.tabs.tabs[i].title
                                            };
                                            for (let j = 0; j < userThursdayFeeds.tabs.tabs[i].subscriptions.length; j++) {
                                                if (userThursdayFeeds.tabs.tabs[i].podcast) {
                                                    sub = podcastFeeds.filter(pod => {
                                                        return (pod.uuid === userThursdayFeeds.tabs.tabs[i].subscriptions[j].uuid);
                                                    });
                                                } else {
                                                    sub = blogFeeds.filter(blo => {
                                                        return (blo.uuid === userThursdayFeeds.tabs.tabs[i].subscriptions[j].uuid);
                                                    });
                                                }
                                                result[i + 1].subscriptions.push({
                                                    uuid: userThursdayFeeds.tabs.tabs[i].subscriptions[j].uuid,
                                                    author: sub[0].author,
                                                    ...
                                                    rss: sub[0].rss,
                                                    title: sub[0].title,
                                                    userRead: userThursdayFeeds.tabs.tabs[i].subscriptions[j].userRead
                                                });
                                            }
                                        }
