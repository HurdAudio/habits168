...
exports.up = function(knex, Promise) {
  return knex.schema.createTable('blog_shares', function(table) {
    table.uuid('uuid').notNullable().primary();
    table.uuid('user_uuid').notNullable().references('uuid').inTable('users').onDelete('CASCADE').index();
    table.uuid('feed_uuid').notNullable().references('uuid').inTable('blog_feeds').onDelete('CASCADE').index();
    table.string('share_status').notNullable().defaultTo('private');
...
exports.down = function(knex, Promise) {
  return knex.schema.dropTable('blog_shares');
};
